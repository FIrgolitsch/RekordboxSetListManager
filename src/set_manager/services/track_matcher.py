...
from set_manager.models.enums import MatchStatus
    ...
    def _match_one(
        ...
        if sp_track.isrc and sp_track.isrc in isrc_index:
            ...
            return MatchResult(
                ...
                local_track=_merge(sp_track, local),
        ...
        if best_local is not None and best_score >= self.FUZZY_THRESHOLD:
            return MatchResult(
                spotify_track=sp_track,
                local_track=_merge(sp_track, best_local),
...
def _merge(spotify_track: Track, local_track: Track) -> Track:
