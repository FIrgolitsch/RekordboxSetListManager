...
class RekordboxXmlService:
    """Export SetLists to Rekordbox XML and import Rekordbox XML track collections."""
    ...
    def export_set(
        ...
        """Write *set_list* to a Rekordbox-importable XML file at *path*.

        Structure:
        - COLLECTION contains every track that appears in the set list.
        - PLAYLISTS contains one top-level folder named after the set list.
        ...
        set_node = ET.SubElement(
            ...
            Type="0",
            Name=set_list.name,
            Count=str(len(set_list.sections)),
        )
        for section in set_list.sections:
...
def _filepath_to_location(filepath: str | None) -> str:
    ...
    p = Path(filepath)
    if p.is_absolute():
        return f"file://localhost{filepath}"
    return filepath


def _location_to_filepath(location: str) -> str | None:
    """Convert a Rekordbox ``Location`` URL to a plain file path."""
    if not location:
        return None
    if location.startswith("file://localhost"):
        return location[len("file://localhost"):]
...
def _elem_to_track(elem: ET.Element) -> Track | None:
    """Convert a TRACK element to a :class:`Track`.  Returns ``None`` if the
    element lacks both a name and an artist (i.e. clearly invalid)."""
